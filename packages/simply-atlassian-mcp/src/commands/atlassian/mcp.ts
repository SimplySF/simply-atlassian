 ...
 *
 * The same server the `simply-atlassian-mcp` binary runs — that binary still exists and still works.
 ...
 *
 * **Extends `Command` directly, not `AtlassianCommand`.** That is deliberate and load-bearing. stdout
 ...
 */
export default class AtlassianMcp extends Command {
  ...
  public static override readonly summary = 'Serve the Atlassian MCP tools over stdio.';
  public static override readonly description =
    'Runs a Model Context Protocol server exposing Jira and Confluence as tools, for Claude Desktop, Claude ' +
    ...
    'Point a client at it with "command": "simply", "args": ["atlassian", "mcp"]. The plugin installs ' +
    'itself on first use, so nothing needs installing beyond the simply CLI.\n\n' +
    'Only read tools are registered unless --allow-writes is passed, and ATLASSIAN_READ_ONLY still ' +
  ...
  public async run(): Promise<void> {
    const { flags } = await this.parse(AtlassianMcp);
