...
the CLI takes a repeatable or comma-separated flag; see the [Command Reference](/reference/) for
what each does. The first request above, for instance, becomes a call like:
...
`--allow-writes` and a write-capable token that is enabled only when a person means to let the
agent write. In a `mcpServers` configuration the pair looks like this:
...
so a wrong key or a missing setting usually ends in the agent telling you what went wrong rather
than in silence.
